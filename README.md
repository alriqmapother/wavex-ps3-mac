# Wave X — the PlayStation 3 XMB wave as a native macOS wallpaper

[![Demo](demo.png)](https://github.com/alriqmapother/wavex-ps3-mac/releases/latest)

Wave X draws the PS3 XrossMediaBar wave live on your desktop with Metal and puts it on the
lock screen too. Menu-bar app, 38 colour variants (12 PS3 month palettes × Day / Night /
Enhanced, plus Black and Custom RGB), a hover-to-preview gallery, and every tuning knob from
the WebGL original.

**[Download the latest DMG →](https://github.com/alriqmapother/wavex-ps3-mac/releases/latest)**
(universal binary, macOS 14+). Drag Wave X into Applications, right-click → Open the first
time (the app is not notarized), then click the wave icon in the menu bar.

- **Live desktop**: a Metal window pinned at the desktop level on every display and Space,
  click-through and behind Finder's icons, at 60 fps with 4× MSAA. Pauses when locked, asleep,
  covered, or when the screensaver runs.
- **Lock screen**: the lock screen can't host a live view, so Wave X renders a per-variant HEVC
  movie with temporal sub-layers and registers it in macOS's aerial wallpaper catalog
  (System Settings → Wallpaper → Landscape → "Wave X"). Verified on macOS 27 / Apple Silicon.
- **Source**: [`WaveX/`](WaveX/) — Swift + Metal + SwiftUI, builds with the Command Line Tools
  alone (`cd WaveX && ./Scripts/build-app.sh`, then `Scripts/make-dmg.sh` for the disk image).
  Details, CLI helpers and lock-screen caveats are in [WaveX/README.md](WaveX/README.md).

The wave itself is a faithful port of the reverse-engineered WebGL implementation described
below, by [linkev](https://github.com/linkev/PlayStation-3-XMB) (MIT), kept in this repo in full.

## Web version and research (upstream project)
...
- `WaveX`: Native macOS live wallpaper + lock-screen app built from `ps3xmbwave` (Swift, Metal, SwiftUI).
